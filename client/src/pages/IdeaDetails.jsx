  ...
  if (loading) {
    return (
      <div className="text-center py-12" role="status" aria-live="polite">
        <Loader2 className="inline-block animate-spin h-12 w-12 text-primary-600 dark:text-primary-400" aria-hidden="true" />
        <p className="mt-4 text-gray-600 dark:text-gray-400">Loading idea details...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12" role="alert">
        <AlertCircle className="inline-block h-12 w-12 text-red-600 dark:text-red-400 mb-4" aria-hidden="true" />
    ...
    <div className="animate-fade-in">
      ...
      <div className="space-y-6">
        <header className="bg-gradient-to-r from-primary-600 to-accent-600 text-white p-8 rounded-2xl shadow-lg">
          ...
          <p className="text-lg text-white/90 leading-relaxed">{idea.description}</p>
        </header>

        {/* Score Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className={`p-6 rounded-xl border-2 transition-all ${getScoreColor(analysis.profitability_score)}`}>
            ...
            <p className="text-5xl font-bold" aria-label={`Profitability score: ${analysis.profitability_score || 0} out of 100`}>
              {analysis.profitability_score ?? "—"}<span className="text-2xl">/100</span>
            </p>
          </div>

          <div className={`p-6 rounded-xl border-2 transition-all ${getRiskColor(analysis.risk_level)}`}>
            <div className="flex items-start justify-between mb-2">
              <div className="p-3 rounded-lg bg-white/50 dark:bg-gray-900/50">
            ...
            <p className="text-5xl font-bold" aria-label={`Risk level: ${analysis.risk_level || 'Unknown'}`}>
              {analysis.risk_level ?? "—"}
            </p>
          </div>
        </div>
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-8">
              ...
              {safeCompetitors().length > 0 ? (
                ...
                <p className="text-gray-600 dark:text-gray-400 italic">No competitor data available</p>
              )}
            </div>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-bold text-xl text-gray-900 dark:text-white">Recommended Tech Stack</h3>
              </div>
              <div className="flex gap-3 flex-wrap">
                {(analysis.tech_stack || []).length > 0 ? (
                  ...
                  <p className="text-gray-600 dark:text-gray-400 italic">No tech stack recommendations</p>
                )}
              </div>
            </div>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-8">
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 p-6 rounded-xl border-l-4 border-primary-500">
                <h3 className="font-bold text-lg text-gray-900 dark:text-white mb-3">AI Analysis Summary</h3>
